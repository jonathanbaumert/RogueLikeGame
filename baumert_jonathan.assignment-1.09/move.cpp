...
void do_combat(dungeon *d, character *atk, character *def)
{
  ...
  if (character_is_alive(def)) {
    if(atk == d->PC){
      pc_dam = atk->damage->roll();
      for(i = 0; i<12; i++){
        if(d->PC->equipment[i]!=0){
          pc_dam += d->PC->equipment[i]->roll_dice(); 
        }
      }
      def->hp -= pc_dam;
      //std::cout<<def->hp<<" "<<d->PC->hp<<std::endl;
      if(def->hp<=0){
        if(def->isBoss==true){
          //d->num_monsters=0;
          d->quit=1;
        ...
        character_increment_ikills(atk,(character_get_ikills(def)+character_get_dkills(def)));
        charpair(def->position) = NULL;        
      }
      pc_dam = 0;
    }
    else if (def==d->PC){
      mon_dam = atk->damage->roll();
      d->PC->hp -= mon_dam;
      if(d->PC->hp <= 0){
        character_die(def);
        character_increment_ikills(atk,(character_get_ikills(def)+character_get_dkills(def)));
      ...
      x = atk->position[dim_x];
      y = atk->position[dim_y];
...
uint32_t move_pc(dungeon *d, uint32_t dir)
{
  ...
  if ((dir != '>') && (dir != '<') && (mappair(next) >= ter_floor)) {
    int i = 0;
    if (d->objmap[next[dim_y]][next[dim_x]] != 0)
    {
      while (i < 10 && d->PC->carry[i] != 0)
      {
        i++;
      }
      if (i < 10)
      {
        d->PC->carry[i] = d->objmap[next[dim_y]][next[dim_x]];
        d->objmap[next[dim_y]][next[dim_x]] = 0;
      }
      else
      {
        io_queue_message("You cannot picked any more items becuase the slots are full");
      }
    }
