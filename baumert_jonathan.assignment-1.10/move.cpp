...
void do_ranged_attack(dungeon *d, character *def){
  uint32_t damage;
  if(character_is_alive(def)){
    damage = d->PC->damage->roll() + d->PC->eq[2]->roll_dice();
    ...
    if (damage >= def->hp) {                
      ...
      character_increment_ikills(d->PC, (character_get_dkills(def) +
    ...
    io_queue_message("Targeted character is no longer alive!");
  }
}
void do_poison_attack(dungeon *d, character *def){
  int x, y, i, j;

  x = def->position[dim_x];
  y = def->position[dim_y];
  if(d->PC->mana>=5){
    if (d->character_map[y][x] != 0 && d->PC->visible[y][x] != 0)
    {
      for (i = y - 2; i < (y + 3); ++i)
      {
        for (j = x - 2; j < (x + 3); ++j)
        {
          ...
          {
            if (d->character_map[i][j] != 0 && d->PC != d->character_map[i][j])
            {
              def = d->character_map[i][j];
              if(character_is_alive(def)){
                uint32_t damage = 10;
                if (damage >= def->hp) {                
                  io_queue_message("You killed %s%s.", is_unique(def) ? "" : "the ", def->name);               
                  ...
                  def->hp -= damage;
                  d->PC->mana-=5;
                  if(d->PC->visible[i][j]!=0){
                    io_queue_message("You poisoned %s%s for %d.", is_unique(def) ? "" : "the ",
                          def->name, 10);
                  }
                }             
              }
    ...
    {
      io_queue_message("Poison failed! There isn't a visible monster to poison in that spot.");
    }
  }
  else{
    io_queue_message("Poison failed! This spell require 5 mana, current mana level is %d", d->PC->mana);
  }
  
}
void do_combat(dungeon *d, character *atk, character *def)
{
  uint32_t damage, i, defense;
  const char *organs[] = {
    "liver",
    "pancreas",
    "heart",
  ...
  if (character_is_alive(def)) {
    if (atk != d->PC) {
      ...
      damage = atk->damage->roll()-defense;
...
void do_moves(dungeon *d)
{
  ...
  if (pc_is_alive(d) && e->c == d->PC) {
    ...
    event_delete(e);
    io_handle_input(d);
    if(d->PC->turn_count%3 == 0){
      d->PC->belly--;
      if(d->PC->belly<=0){
        d->PC->alive=0;
        io_queue_message("You died of starvation!");
      }
      else if(d->PC->belly==50){
        io_queue_message("Your belly is half full!");
      }
      else if(d->PC->belly==10){
        io_queue_message("Your belly is nearly empty eat something soon to prevent starving!");
      }
    }
    else if(d->PC->turn_count%2==0){
      if(d->PC->mana<100){
        d->PC->mana++;
      }
    }
