...
static uint32_t io_target_monster(dungeon *d)
{
  pair_t dest, tmp;
  int c;
  fd_set readfs;
  ...
  mvprintw(0, 0, "Target a monster.  't' or '+ to target; 'ESC' to cancel.");
  ...
  * not otherwise used.                                                 */
  ...
  target = charpair(dest);

  mvprintw(2, 0, "Hit any key to continue. ");
...
uint32_t io_ranged_attack(dungeon *d){
  if(target!=0){
    
    if(d->PC->eq[2]!=0){
      if(target==d->PC){
         io_queue_message("PC position chosen. You can't attack yourself, silly!");   
         return 1;
      }
      else{
        int x,y;
        x = target->position[dim_x];
        y = target->position[dim_y];
        if(d->PC->visible[y][x]==0){
          io_queue_message("Target is outside visible range! Attack failed!");
          return 1;
        }
        if(!character_is_alive(target)){
          io_queue_message("Target is no longer alive! Attack failed!");
          return 1;
        }
        do_ranged_attack(d,target);
        return 0;
      }
    }
    else{
      io_queue_message("You must have a ranged weapon equipped to perform a ranged attack!"); 
      return 1;  
    }
  }
  else{
    io_queue_message("No valid target selected");
    return 1;
  }
}
uint32_t io_poison_attack(dungeon *d){
  if(target!=0){    
    if(target==d->PC){
        io_queue_message("PC position chosen. You can't attack yourself, silly!");   
        return 1;
...
uint32_t io_expunge_in(dungeon *d)
{
  ...
  mvprintw(16, 10, " %-58s ", "");
  mvprintw(17, 10, " %-58s ", "Destroy or eat which item (ESC to cancel)?");
  refresh();
