...
void io_pc_drop_item(dungeon *d)
{
  ...
  object *obj, *obj2;
  const char *name;
  char msg[52] = "Enter the number of the carried item to be dropped";
  char msg2[29] = "Enter any other key to exit";
  io_clear_screen();
  mvprintw(5, 10, msg);
  mvprintw(6, 10, msg2);
  for (i = 0; i < 10; i++)
  {
    if (d->PC->carry[i] != 0)
    {
      name = d->PC->carry[i]->get_name();
    }
    else
    {
      name = "EMPTY SLOT";
    }
    mvprintw(i + 10, 10, "%d%c %s", i, '.', name);
  }
  i = getch();
  i = convert_to_int('c', i);
  obj = io_select_inventory(d, i);
  obj2 = 0;
  if (obj != 0)
  {
    ...
    if (d->objmap[y][x] != 0)
    {
      obj2 = d->objmap[y][x];
      name = obj2->get_name();
      mvprintw(4, 10, "You picked up a %s", name);
      d->objmap[y][x] = obj;
      name = obj->get_name();
      mvprintw(5, 10, "You dropped up a %s", name);
      d->PC->carry[i] = obj2;
    }
    else
    {
  ...
  {
    io_clear_screen();
    mvprintw(5, 10, "%s", "Slot was either not valid or empty");
  }
  mvprintw(6, 10, "%s", "Press anything to continue");
  refresh();
  i = getch();
  io_display(d);
}
...
void io_pc_delete_item(dungeon *d)
{
  ...
  int i;
  const char *name;
  char msg[52] = "Enter the inventory item number you want to delete";
  char msg2[39] = "Press any non inventory key to return";
  mvprintw(23, 5, "%s", msg);
  mvprintw(24, 5, "%s", msg2);
  for (i = 0; i < 10; i++)
  {
    if (d->PC->carry[i] != 0)
    {
  ...
  {
    name = obj->get_name();
    mvprintw(5, 10, "The object deleted was a %s", name);
    //io_queue_message("The object deleted was a %s",name);
    delete obj;
    for (i = 1; i < 10; ++i)
    {
      d->PC->carry[i - 1] = d->PC->carry[i];
    }
    d->PC->carry[9] = 0;
...
void io_display_inventory(dungeon *d)
{
  int i;
  const char *name;
  char msg[28] = "Press anything to continue";
  io_clear_screen();
  for (i = 0; i < 10; i++)
  {
    if (d->PC->carry[i] != 0)
...
void io_display_equipment(dungeon *d)
{
  ...
  for (i = 0; i < 12; i++)
  {
    ...
    mvprintw(i + 5, 10, "%c%c %s %s", ch, '.', type, name);
  }
...
void io_inspect_item(dungeon *d)
{
  ...
  object *obj;
  char msg[41] = "Select an item to view it's description";
  io_clear_screen();
  mvprintw(4, 5, msg);
  for (i = 0; i < 10; i++)
  {
  ...
  if (obj != 0)
  {
    ...
    mvprintw(4, 5, "Item name:   %s", name);
    mvprintw(5, 5, "Description: %s", description);
  }
  else
  {
    mvprintw(5, 5, "Invalid or empty slot selected, no item to display");
  }
...
void io_inspect_monster(dungeon *d)
{
  ...
  mvprintw(0, 0,
           "Choose a location.  't' inspect monster; escape to return to game");
  ...
  * not otherwise used.                                                 */
  ...
  {
    charact = d->character_map[dest[dim_y]][dest[dim_x]];
    if (charact != 0 && charact != d->PC)
    {
      io_clear_screen();
      n = (npc *)d->character_map[dest[dim_y]][dest[dim_x]];
      mvprintw(3, 5, "Monster HP:          %d", n->hp);
      mvprintw(4, 5, "Monster speed:       %d", n->speed);
      mvprintw(5, 5, "Monster description: %s", n->description);
    }
    else
    {
      mvprintw(5, 5, "No valid monster at that position!");
    }
  }
  mvprintw(15, 5, "Press any key to return to game");
  refresh();
  getch();
  io_display(d);
}
void equip_object(object *obj, dungeon *d, int p)
{
  int type;
  type = obj->get_type();
  ...
  if (type == 1)
  {
    if (d->PC->equipment[0] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[0];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[0] = obj;
  }
  else if (type == 2)
  {
    if (d->PC->equipment[1] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[1];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[1] = obj;
  }
  else if (type == 3)
  {
    if (d->PC->equipment[2] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[2];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[2] = obj;
  }
  else if (type == 4)
  {
    if (d->PC->equipment[9] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[9];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[9] = obj;
  }
  else if (type == 5)
  {
    if (d->PC->equipment[3] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[3];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[3] = obj;
  }
  else if (type == 6)
  {
    if (d->PC->equipment[4] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[4];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[4] = obj;
  }
  else if (type == 7)
  {
    if (d->PC->equipment[5] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[5];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[5] = obj;
  }
  else if (type == 8)
  {
    if (d->PC->equipment[6] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[6];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[6] = obj;
  }
  else if (type == 9)
  {
    if (d->PC->equipment[7] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[7];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[7] = obj;
  }
  else if (type == 10)
  {
    if (d->PC->equipment[8] != 0)
    {
      d->PC->carry[p] = d->PC->equipment[8];
    }
    else
    {
      d->PC->carry[p] = 0;
    }
    d->PC->equipment[8] = obj;
  }
  else if (type == 11)
  {
    if (d->PC->equipment[10] != 0)
    {
      if (d->PC->equipment[11] != 0)
      {
        d->PC->carry[p] = d->PC->equipment[11];
      }
      else
      {
        d->PC->carry[p] = 0;
      }
      d->PC->equipment[11] = obj;
    }
    else
    {
      d->PC->equipment[10] = obj;
    }
  }
  update_pc_speed(d);
}
...
void io_wear_item(dungeon *d)
{
  int i;
  const char *name;
  char msg[61] = "Enter the number of the object to wear, hit esc to continue";
  object *obj;
  obj = 0;
  io_clear_screen();
  mvprintw(4, 5, msg);
  ...
  if (obj != 0)
  {
    ...
    if (d->PC->carry[i] != 0)
    {
      ...
      mvprintw(5, 5, "You are now wearing: %s", name);
    }
  }
  else
  {
    mvprintw(5, 5, "INVALID OR EMPTY SLOT SELECTED");
  }
...
void io_takeoff_item(dungeon *d)
{
  ...
  object *obj;
  char ch;
  char msg[37] = "Select the slot you wih to take off";
  char msg2[29] = "Press any other key to exit";
  io_clear_screen();
  mvprintw(2, 5, msg);
  mvprintw(2, 3, msg2);
  for (i = 0; i < 12; i++)
  ...
  refresh();
  i = getch();
  i = convert_to_int('e', i);
  obj = io_select_equipment(d, i);
  io_clear_screen();
  if (obj != 0)
  {
    //check for open slot
    j = 0;
    while (j < 10 && d->PC->carry[j] != 0)
    {
      j++;
    }
    if (j < 10)
    {
      ...
      d->PC->carry[j] = obj;
      d->PC->equipment[i] = 0;
      mvprintw(5, 5, "You put away: %s", name);
      update_pc_speed(d);
    }
    else
    {
      mvprintw(5, 5, "NO OPEN INVENTORY SLOTS, PLEASE DROP AN ITEM");
    }
  }
  else
  {
