...
void io_display_fog(dungeon_t *d)
{
  ...
  for (y = 0; y < 21; y++)
  {
    for (x = 0; x < 80; x++)
    {
      if ((d->pc_visible[y][x])||(d->pc_known_terrain[y][x]!=ter_cantsee))
      {
        if (d->character[y][x])
        {
...
uint32_t io_teleport_pc(dungeon_t *d)
{
  io_display(d);
  /* Just for fun. */
  pair_t original_position;
  original_position[dim_x] = d->pc.position[dim_x];
  original_position[dim_y] = d->pc.position[dim_y];
  pair_t dest;
  dest[dim_x] = original_position[dim_x];
  dest[dim_y] = original_position[dim_y];
  //uint32_t fail_code;
  int key;
...
static void io_scroll_monster_list(char(**s), uint32_t count)
{
  uint32_t offset;
  uint32_t i;
