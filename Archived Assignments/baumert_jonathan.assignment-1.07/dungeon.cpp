...
void pc_start_known_terrain(dungeon_t *d){
  ...
  for(x=0;x<DUNGEON_X;x++){
    for(y=0;y<DUNGEON_Y;y++){
      d->pc_known_terrain[y][x] = ter_cantsee;
      d->pc_visible[y][x]=0;
    }
  }
}
void pc_observe_terrain(dungeon_t *d){
  ...
  min_y = d->pc.position[dim_y] - PC_VISIBLE_RANGE;
  min_x = d->pc.position[dim_x] - PC_VISIBLE_RANGE;
  max_y = d->pc.position[dim_y] + PC_VISIBLE_RANGE;
  max_x = d->pc.position[dim_x] + PC_VISIBLE_RANGE;
  if(min_y<0){min_y=0;}
  if(min_x<0){min_x=0;}
  if(max_y>DUNGEON_Y-1){max_y=DUNGEON_Y-1;}
  if(max_x>DUNGEON_X-1){max_x=DUNGEON_X-1;}
  for (loc[dim_y]=min_y;loc[dim_y]<=max_y;loc[dim_y]++){
    loc[dim_x] = min_x;
    can_see_pair(d, d->pc.position,loc);
    loc[dim_x] = max_x;
    can_see_pair(d, d->pc.position,loc);
  }
  for (loc[dim_x]=min_x-1;loc[dim_x]<=max_x-1;loc[dim_x]++){
    loc[dim_y] = min_y;
    can_see_pair(d, d->pc.position,loc);
    loc[dim_y] = max_y;
    can_see_pair(d, d->pc.position,loc);
...
int gen_dungeon(dungeon_t *d)
{
  ...
  connect_rooms(d);
  place_stairs(d);
  pc_start_known_terrain(d);
  pc_observe_terrain(d);
  return 0;
}
