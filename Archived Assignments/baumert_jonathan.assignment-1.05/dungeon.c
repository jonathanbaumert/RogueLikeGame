...
void display_monsters(dungeon_t *d){
  ...
  int monsters_locations[d->num_monsters][2];
  char *direction[d->num_monsters][2];
  int pc_x = d->pc.position[dim_x];
  int pc_y = d->pc.position[dim_y];
  char *menu = malloc(d->num_monsters*20);

  pair_t p;
  //character_t curr_mon;
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
    ...
    for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
      ...
      if (charpair(p)!=NULL){
        //printf("HERE\n");
        if(d->character[p[dim_y]][p[dim_x]]!=&d->pc){
          monsters[monster_counter] = *d->character[p[dim_y]][p[dim_x]];
          
          if(p[dim_x]>= pc_x){
            monsters_locations[monster_counter][dim_x]=p[dim_x]-pc_x;
            direction[monster_counter][dim_x]="EAST";
          }
          else{
            monsters_locations[monster_counter][dim_x]=pc_x-p[dim_x];
            direction[monster_counter][dim_x]="WEST";
          }
          if(p[dim_y]>=pc_y){
            monsters_locations[monster_counter][dim_y]=p[dim_y]-pc_y;
            direction[monster_counter][dim_y]="SOUTH";
          }
          else{
            monsters_locations[monster_counter][dim_y]=pc_y-p[dim_y];
            direction[monster_counter][dim_y]="NORTH";
          }
          sprintf(menu+(20*monster_counter),"%c %s %2d, %s %2d  ",monsters[monster_counter].symbol,
                                                                  direction[monster_counter][dim_y],
                                                                  monsters_locations[monster_counter][dim_y],
                                                                  direction[monster_counter][dim_x],
                                                                  monsters_locations[monster_counter][dim_x]);
          monster_counter++;
        }
      }
    }
  ...
  for(i = 0; i<min;i++){
    for(j = 0; j<20; j++){
      mvaddch(i+21,j+2,menu[j+monster_counter*20]);
    }
    monster_counter++;
  }
  refresh();
  ...
  while (ch!=27){    
    ...
    if(ch == KEY_UP){
      if(start>0){
        start--;
        monster_counter = start;
        
        for(i = 0+start; i<min+start;i++){
          for(j = 0; j<20; j++){
            mvaddch(i+21-start,j+2,menu[j+monster_counter*20]);
          }
          monster_counter++;
        }
...
static int put_stairs(dungeon_t *d){
  int x, y, i, j, random;
  i = rand()%2 + 1;
  for(j = 0; j<i; ++j){
    //place up stairs
    random = rand()%d->num_rooms+1;
    x=d->rooms[random].position[dim_x];
    y=d->rooms[random].position[dim_y];
    d->map[y][x]=ter_up_stair;
    //place down stairs
    random=rand()%d->num_rooms +1;
    x=d->rooms[random].position[dim_x];
    y=d->rooms[random].position[dim_y];
    d->map[y][x]=ter_down_stair;
  }
  return 0;
}
